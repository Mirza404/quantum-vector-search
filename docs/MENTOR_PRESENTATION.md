...
> "We built a benchmarking system that compares classical and quantum vector search algorithms side by side. You type a text query, and the system finds matching images using CLIP embeddings. We then ran five different search engines on the same data — two classical, three quantum — and measured accuracy and algorithmic cost. The quantum engines run on a real quantum circuit simulator. We found that the quantum algorithms produce correct results but cannot beat classical today, and we documented exactly why."
...
- Live demo: type a query, see results from all five engines side by side
