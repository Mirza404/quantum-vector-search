...
> "We built a benchmarking system that compares classical and quantum vector search algorithms side by side. The user selects from predefined ground-truth queries and chooses which engines to run; the system finds matching images using CLIP embeddings. We benchmarked five search engines on the same data - three classical, two quantum - and measured accuracy and algorithmic cost. The quantum engines run on a real quantum circuit simulator. We found that the quantum algorithms produce correct results but cannot beat classical today, and we documented exactly why."
