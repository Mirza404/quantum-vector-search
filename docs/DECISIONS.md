...
- **Beans → Flickr30k subset (Mar 23, 2026):** To regain a more diverse corpus, we replaced the `datasets.load_dataset` call with a direct download of `flickr_annotations_30k.csv` and `flickr30k-images.zip` via `huggingface_hub`. The importer now samples `num_images` rows with `random_state=42`, extracts the matching files from the archive, and writes the captions into `ground_truth.jsonc`. This restores Flickr30k coverage without relying on `trust_remote_code` while keeping the rest of the benchmarking flow identical.
