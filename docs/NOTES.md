...
* **Implementation:** `DirectoryDataLoader` — reads WebP images from `backend/data/images/`. Ground truth is loaded from `backend/data/ground_truth.jsonc`, generated by `backend/scripts/import_dataset.py` (Flickr30k via HuggingFace).
...
CLI flags (`--shots-values`, `--layers-values`, `--dimensions`) override the YAML values for one-off runs.
