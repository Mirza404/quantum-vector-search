...
| Size | A0 (841 x 1189 mm, ~33 x 47 in) |
...
> actually only 1.4x taller than wide. We picked A0 because that is what the
...
| QR code | Bottom-right linking to repository (generate at build time, see Sec. QR) |
...
| Table: 7 active engines + IBM validation | MRR formula, oracle-call defn |
...
2. Iterate on `index.html` + `style.css`. Use Chrome DevTools device mode at
   841 x 1189 px to preview at-scale.
...
print export uses the raw 841 x 1189 mm size.
...
Per handbook Sec. 2.3, the poster is uploaded to Microsoft Teams as a PDF
