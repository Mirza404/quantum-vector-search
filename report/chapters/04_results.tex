...
\begin{table}[htbp]
  \centering
  \caption[Mean Reciprocal Rank by engine and dimension]{Average MRR by engine and embedding
...
The MRR values in Table~\ref{tab:mrr} are computed with
Equation~\ref{eq:mrr} of Section~\ref{sec:method:metrics}, averaged over the
twenty benchmark queries. Table~\ref{tab:mrr} shows that brute-force cosine,
FAISS flat, and FAISS HNSW all scored MRR 0.829 and produced the same
query-by-query ordering. This is
...
\begin{figure}[H]
  \centering
  % TODO (team): replace with Elhan's IBM hardware diagram(s) from Teams.
  % Drop the exported image at figures/ibm_hardware.png and swap \placeholderfig
  % for: \includegraphics[width=0.85\linewidth]{figures/ibm_hardware.png}
  \placeholderfig{0.85\linewidth}{IBM Quantum hardware validation: transpiled swap-test circuit and/or the measured results from the IBM Quantum run.}
  \caption[IBM Quantum hardware validation run]{IBM Quantum hardware validation run: the swap-test circuit as executed on the physical device and its measured outcome (dimension 2, 2 candidates, 32 shots).}
  \label{fig:ibm_hardware}
\end{figure}
...
% The Limitations section has been moved to the Conclusion chapter (Section 6.2)
% per the supervisor's review, which asked for the structure
